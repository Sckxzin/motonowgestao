...
app.get('/clientes/buscar', auth, async (req, res) => {
  ...
  const like = `%${q}%`;
  const c = await db.one(`SELECT c.*,cm.chassi,cm.modelo as moto_modelo,cm.cor as moto_cor,cm.ano as moto_ano
    FROM clientes c LEFT JOIN cliente_motos cm ON cm.cliente_id=c.id
    WHERE c.telefone ILIKE $1 OR c.nome ILIKE $1 OR c.cpf ILIKE $1 LIMIT 1`, [like]);
  res.json(c || null);
});
...
app.get('/pecas', auth, async (req, res) => {
  ...
  const filtro = role==='FILIAL' ? userCidade : (cidade||null);
...
app.put('/pecas/:id/estoque', auth, adminOnly, async (req, res) => {
  ...
  const r = await db.one('UPDATE pecas SET estoque=estoque+$1,preco=$2 WHERE id=$3 RETURNING *',
    [Number(quantidade), np, req.params.id]);
  res.json(r);
});
app.post('/pecas/transferir', auth, adminOnly, async (req, res) => {
...
app.post('/motos/vender', auth, async (req, res) => {
  ...
  try {
    ...
    const comissao = calcularComissao(moto.modelo, valor);
    let cliId = cliente_id||null;
    if (!cliId) {
      const c = await client.query('SELECT id FROM clientes WHERE telefone=$1 OR nome=$2 LIMIT 1', [numero_cliente, nome_cliente]);
      if (c.rows[0]) cliId = c.rows[0].id;
      else { const nc = await client.query('INSERT INTO clientes(nome,cpf,telefone,cidade) VALUES($1,$2,$3,$4) RETURNING id', [nome_cliente, cpf||null, numero_cliente||null, filial_venda]); cliId=nc.rows[0].id; }
    }
    if (cliId) {
      const jaTem = await client.query('SELECT id FROM cliente_motos WHERE cliente_id=$1 AND chassi=$2', [cliId, moto.chassi]);
      if (!jaTem.rows[0]) await client.query('INSERT INTO cliente_motos(cliente_id,chassi,modelo,cor,ano) VALUES($1,$2,$3,$4,$5)', [cliId, moto.chassi, moto.modelo, moto.cor, moto.ano_moto]);
    }
...
app.get('/vendas-motos', auth, adminOnly, async (_, res) => {
  res.json(await db.q('SELECT * FROM vendas_motos ORDER BY created_at DESC'));
});
app.put('/vendas-motos/:id', auth, adminOnly, async (req, res) => {
  const { valor, rp, rr, forma_pagamento, comissao_valor } = req.body;
  const r = await db.one('UPDATE vendas_motos SET valor=COALESCE($1,valor),rp=COALESCE($2,rp),rr=COALESCE($3,rr),forma_pagamento=COALESCE($4,forma_pagamento),comissao_valor=COALESCE($5,comissao_valor) WHERE id=$6 RETURNING *',
    [valor?Number(valor):null, rp!=null?rp?1:0:null, rr!=null?rr?1:0:null, forma_pagamento||null, comissao_valor?Number(comissao_valor):null, req.params.id]);
  res.json(r);
});
...
app.post('/os/:id/itens', auth, async (req, res) => {
  ...
  try {
    ...
    let baixou = 0;
    if (peca_id) {
      const p = (await client.query('SELECT estoque, nome FROM pecas WHERE id=$1 FOR UPDATE', [peca_id])).rows[0];
...
// ════════════════════════════════════════════
// Migração: cria tabela de checklist
;(async()=>{
  try {
    await db.run(`CREATE TABLE IF NOT EXISTS os_checklist (
    ...
    await db.run(`CREATE TABLE IF NOT EXISTS os_orcamento (
      id SERIAL PRIMARY KEY,
      os_id INTEGER NOT NULL REFERENCES ordens_servico(id) ON DELETE CASCADE,
...
const CHECKLIST_PADRAO = [
  'Óleo do motor','Filtro de óleo','Filtro de ar','Vela de ignição',
  'Corrente e coroa','Pneu dianteiro','Pneu traseiro','Freio dianteiro',
  'Freio traseiro','Fluido de freio','Suspensão dianteira','Suspensão traseira',
  'Farol e lanternas','Buzina','Espelhos retrovisores','Cabo de acelerador',
...
app.get('/os/:id/checklist', auth, async (req, res) => {
  ...
  if (items.length === 0) {
    ...
    for (const item of CHECKLIST_PADRAO) {
      await db.run('INSERT INTO os_checklist(os_id,item,status) VALUES($1,$2,$3)', [req.params.id, item, 'OK']);
    }
...
app.put('/os/:id/checklist', auth, async (req, res) => {
  const { items } = req.body; // [{id, status, observacao}]
  for (const it of items) {
    await db.run('UPDATE os_checklist SET status=$1, observacao=$2 WHERE id=$3 AND os_id=$4',
      [it.status, it.observacao||null, it.id, req.params.id]);
  }
  res.json({ ok: true });
});
...
app.post('/os/:id/orcamento', auth, async (req, res) => {
  ...
  await db.run('INSERT INTO os_orcamento(os_id,aprovado,aprovado_em,observacao) VALUES($1,$2,NOW(),$3)',
    [req.params.id, aprovado?1:0, observacao||null]);
  // atualiza status da OS
  if (aprovado) {
    await db.run("UPDATE ordens_servico SET status='EM_ANDAMENTO' WHERE id=$1 AND status='ABERTA'", [req.params.id]);
  } else {
    await db.run("UPDATE ordens_servico SET status='CANCELADA' WHERE id=$1", [req.params.id]);
  }
  res.json({ ok: true });
});

app.get('/os/:id/orcamento', auth, async (req, res) => {
  const orc = await db.one('SELECT * FROM os_orcamento WHERE os_id=$1', [req.params.id]);
  res.json(orc || null);
});
...
app.get('/oficina/dashboard', auth, async (req, res) => {
  ...
  const [abertas, andamento, aguardando, finalizadas, faturamento, tempoMedio, pecasMaisUsadas] = await Promise.all([
    db.one(`SELECT COUNT(*) as n FROM ordens_servico ${w} AND status='ABERTA'`.replace('AND','WHERE').replace('WHERE AND','WHERE'), p),
    ...
    db.q(`SELECT nome_peca, SUM(quantidade) as total_usado, COUNT(*) as qtd_os FROM os_itens oi
      JOIN ordens_servico os ON os.id = oi.os_id
      WHERE 1=1 ${wa.replace('cidade','os.cidade')}
      GROUP BY nome_peca ORDER BY total_usado DESC LIMIT 10`, p),
  ]);

  res.json({
    abertas: Number(abertas.n),
    em_andamento: Number(andamento.n),
    aguardando: Number(aguardando.n),
    finalizadas: Number(finalizadas.n),
    faturamento_total: Number(finalizadas.total),
    faturamento_mes: Number(faturamento.fat),
    tempo_medio_horas: Number(tempoMedio.horas).toFixed(1),
...
app.get('/os/sugestoes/:modelo', auth, async (req, res) => {
  ...
  const { cidade } = req.user;
  // pega as peças mais usadas em OS do mesmo modelo
  const sugestoes = await db.q(`
    SELECT oi.nome_peca, SUM(oi.quantidade) as total, AVG(oi.preco_unitario) as preco_medio,
      p.id as peca_id, p.estoque, p.preco
    FROM os_itens oi
    JOIN ordens_servico os ON os.id = oi.os_id
    LEFT JOIN pecas p ON p.nome = oi.nome_peca AND p.cidade = $2
    WHERE os.modelo_moto ILIKE $1
    GROUP BY oi.nome_peca, p.id, p.estoque, p.preco
