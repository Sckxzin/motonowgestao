...
export default function OSDetalhe() {
  ...
  const pecasFiltradas = pecaBusca.length >= 2 ? pecasDisponiveis.filter(p => p.nome.toLowerCase().includes(pecaBusca.toLowerCase()) && p.estoque > 0).slice(0, 8) : [];
    ...
    <div className="page">
      ...
      <div className="pc">
        ...
        <div style={{ display:'flex', gap:4, marginBottom:16, borderBottom:'1px solid var(--bd)' }}>
          {[{k:'servico',l:'📋 Serviço'},{k:'checklist',l:'🔍 Checklist' + (checkTrocar > 0 ? ' ('+checkTrocar+' ❌)' : '')},{k:'pecas',l:'📦 Peças ('+((os.itens||[]).length)+')'},{k:'orcamento',l:'💰 Orçamento'}].map(a => (
              ...
              <div className="card">
                  ...
                  <div style={{ marginBottom:14 }}>
                    ...
                    <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                      {sugestoes.filter(s => s.peca_id && s.estoque > 0).slice(0,5).map(s => (
                        <button key={s.nome_peca} className="ab" style={{ fontSize:11 }} onClick={() => adicionarPeca({ id: s.peca_id, nome: s.nome_peca, preco: s.preco || s.preco_medio })}>+ {s.nome_peca} ({formatBRL(s.preco || s.preco_medio)})</button>
                      ))}
                    </div>
                  </div>
                )}
                {!finalizada && (
                  <div style={{ position:'relative', marginBottom:12 }}>
                    <input className="inp" placeholder="Buscar peça..." value={pecaBusca} onChange={e=>setPecaBusca(e.target.value)} />
                    {pecasFiltradas.length > 0 && (
                      <div style={{ position:'absolute', top:'100%', left:0, right:0, background:'var(--s2)', border:'1px solid var(--bd2)', borderRadius:'var(--r)', zIndex:100, maxHeight:200, overflowY:'auto' }}>
                        {pecasFiltradas.map(p => (
                          <div key={p.id} onClick={() => adicionarPeca(p)} style={{ padding:'10px 14px', cursor:'pointer', fontSize:13, display:'flex', justifyContent:'space-between', borderBottom:'1px solid var(--bd)' }} onMouseEnter={e=>e.currentTarget.style.background='var(--s3)'} onMouseLeave={e=>e.currentTarget.style.background=''}>
                ...
                {(os.itens||[]).length === 0 ? <p style={{ fontSize:13, color:'var(--tx3)', textAlign:'center', padding:'16px 0' }}>Nenhuma peça adicionada.</p> : (
                  <div className="tw">
                    <table className="t">
                      ...
                      <tbody>
                        {os.itens.map(it => (
                          <tr key={it.id}>
                            ...
                            <td>{it.quantidade}</td><td>{formatBRL(it.preco_unitario)}</td>
      ...
      <div ref={printRef} style={{ display:'none' }}>
        ...
        {(os.itens||[]).length > 0 && <><h2>Peças</h2><table><thead><tr><th>Peça</th><th>Qtd</th><th>Unit.</th><th>Subtotal</th></tr></thead><tbody>{os.itens.map(it=><tr key={it.id}><td>{it.nome_peca}</td><td>{it.quantidade}</td><td>R$ {Number(it.preco_unitario).toFixed(2)}</td><td>R$ {(it.preco_unitario*it.quantidade).toFixed(2)}</td></tr>)}</tbody></table></>}
        {checklist.length > 0 && <><h2>Checklist</h2><table><thead><tr><th>Item</th><th>Status</th></tr></thead><tbody>{checklist.map(it=><tr key={it.id}><td>{it.item}</td><td className={it.status==='OK'?'ok':it.status==='ATENCAO'?'atencao':'trocar'}>{it.status==='OK'?'✅ OK':it.status==='ATENCAO'?'⚠️ Atenção':it.status==='TROCAR'?'❌ Trocar':'N/A'}</td></tr>)}</tbody></table></>}
